  ...
  <section class="cart_module">
    <section v-if="!foods.specifications.length" class="cart_button">
      <transition name="showReduce">
        <span @click="removeOutCart(foods.cat_id, foods.item_id, foods.product_id, foods.name, foods.price, '', foods.bn, foods.store)" v-if="foodNum">
          <svg>
      ...
      <svg class="add_icon" @touchstart="addToCart(foods.cat_id, foods.item_id, foods.product_id, foods.name, foods.price, '', foods.bn, foods.store, $event)">
...
export default {
  ...
  methods: {
    ...
    removeOutCart (category_id, item_id, food_id, name, price, specs, sku_id, stock) {
      if (this.foodNum > 0) {
        this.REDUCE_CART({category_id, item_id, food_id, name, price, specs, sku_id, stock})
      }
    },
    // 加入购物车，计算按钮位置。
