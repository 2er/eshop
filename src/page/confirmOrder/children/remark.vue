    ...
    <section v-if="!showLoading">
      ...
      <section class="quick_remark" v-if="remarkList.length">
        ...
        <ul class="remark_arr_list_ul">
          <li class="remark_arr_list_li" v-for="(item,index) in remarkList" :key="index">
...
import {markList} from '../../../service/getData'
...
export default {
  ...
  methods: {
    ...
    // 初始化信息
    async initData () {
      let markListData = await markList({store_id: this.storeId})
      if (markListData.return_code === '0000') {
        this.remarkList = markListData.data.marks
      }
