<template>
  <div>
    <section v-if="!showLoading" class="shop_container">
      <head-top head-title="商品列表" goBack="true">
      </head-top>
      <section class="food_container">
        <section class="menu_container">
          <section class="menu_left" id="wrapper_menu" ref="wrapperMenu">
            <ul>
              <li v-for="(item,index) in dishes" :key="index" class="menu_left_li" :class="{activity_menu: index == catIndex}" @click="chooseMenu(index)">
              ...
              <li v-for="(item,index) in dishes" :key="index">
                <header class="menu_detail_header">
                  <section class="menu_detail_header_left">
                    <strong class="menu_item_title">{{item.cat_name}}</strong>
                    <span class="menu_item_description">{{item.addon}}</span>
                  </section>
                ...
                <section v-for="(foods,foodindex) in item.goods" :key="foodindex" class="menu_detail_list">
                  ...
                  <footer class="menu_detail_footer">
                    ...
                    <buy-cart :foods='foods' @moveInCart="listenInCart" @showChooseList="showChooseList" @showReduceTip="showReduceTip" @showMoveDot="showMoveDotFun"></buy-cart>
                  </footer>
                </section>
              </li>
            </ul>
        ...
        <section class="buy_cart_container">
          ...
          <section class="gotopay" :class="{gotopay_acitvity: cartFoodList.length}">
            <router-link :to="{path:'/confirmOrder'}" class="gotopay_button_style" v-if="cartFoodList.length > 0" >去结算</router-link>
            <span class="gotopay_button_style" v-else>请添加</span>
          </section>
        </section>
...
import { goods } from '../../service/getData'
...
export default {
  ...
  computed: {
    ...
    // 根据当前选中规格确定的产品
    choosedProduct: function () {
      let product = {product_id: '', price: 0.00, bn: '', name: '', store: ''}
      this.choosedFoods.forEach(choosedFoodItem => {
        let checked = true
        let specDesc = choosedFoodItem.spec_desc.spec_value_id
        specDesc.forEach((specId, specValueId) => {
          if (this.specsIndex[specId] !== specValueId) {
            checked = false
            return false
          }
        })
        if (checked === true) {
          product = choosedFoodItem
          return false
        }
      })
      return product
  ...
  methods: {
    ...mapMutations([
      'ADD_CART', 'REDUCE_CART', 'INIT_BUYCART', 'CLEAR_CART', 'RECORD_SHOPDETAIL'
    ]),
    // 初始化时获取基本数据
    async initData () {
      // 获取商铺食品列表
      let dishesData = await goods()
      if (dishesData.return_code === '0000') {
        this.dishes = dishesData.data.list
      }
      // 隐藏加载动画
      this.hideLoading()
    },
    ...
    addToCart (category_id, item_id, food_id, name, price, specs) {
      this.ADD_CART({category_id, item_id, food_id, name, price, specs})
    },
    // 移出购物车，所需7个参数，商铺id，食品分类id，食品id，食品规格id，食品名字，食品价格，食品规格
    removeOutCart (category_id, item_id, food_id, name, price, specs) {
      this.REDUCE_CART({category_id, item_id, food_id, name, price, specs})
    },
    /**
    ...
    initCategoryNum () {
      ...
      this.dishes.forEach((item, index) => {
        if (this.shopCart && this.shopCart[item.cat_id]) {
          let num = 0
    ...
    // 记录当前所选规格的索引值
    chooseSpecs (specId, specValueId) {
      this.specsIndex[specId] = specValueId
    },
    // 多规格商品加入购物车
    addSpecs (category_id, item_id, food_id, name, price, specs, packing_fee, sku_id, stock) {
      this.ADD_CART({category_id, item_id, food_id, name, price, specs, packing_fee, sku_id, stock})
      this.showChooseList()
    },
    goback () {
      this.$router.go(-1)
